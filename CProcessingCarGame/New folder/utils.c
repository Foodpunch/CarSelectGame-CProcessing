...
{
	buttonIndex = 0;
	memset(ButtonArray, 0, sizeof(Button) * 100);
}


//##############################|| PHYSICS STUFF || #####################################################################

//Creates a rigidbody with the collider, mass and constraints provided
...
//Creates a collider using shape provided
...
//Creates a physics object and adds it to the physics object array
...
//Function that checks collision with ALL physics object in the array
void UpdatePhysics()
{
...
//Function that updates phaysics object. Called in physics update loop
...
{	
	...
	DrawTriangleAdvanced(GetPointInCircle(obj->rigidBody.collider.shape, 40), GetPointInCircle(obj->rigidBody.collider.shape, 140), GetPointInCircle(obj->rigidBody.collider.shape, 270), obj->rigidBody.collider.shape.transform.rotation);
}
	...
	for (int i = 0; i < physicsObjectIndex - 1; i++)
	{
		...
		for (int j = i + 1; j < physicsObjectIndex; j++)
		{
			...
			if (IsCircleIntersecting(objA->rigidBody.collider.shape, objB->rigidBody.collider.shape))
			{
				...
				//Creates too much nauseating screenshake :c 
				//trauma += 0.15f;
			}
